...
import {SingleNote,MasonaryLayout,Sidebar}from '../../components/index'
import './labelPage.css'
import {useNote,useTheme} from '../../context/index'
        ...
        <div className={isSidebar ? "notes-page margin-left" :"notes-page"}>
            <div className='notes-container'>
                <div className='notes-render-div'>
                        ...
                        <ul className='filter-tags-container'>
                            {uniqueTags.map((tag,i)=>(
                                <li key={i} className="tag-btn">
                                    <label htmlFor={tag}>
                                        <input type="checkbox" id={tag} value={tag} checked={state.newTags.includes(tag)}
                                        onChange={()=>dispatch({type:"FILTER_BY_TAG",payload:tag})}/>
                                      {tag}
                                    </label>
                    ...
                    <MasonaryLayout>
                        {newList.map((note,i)=>{
                            return note.tags.length >0 && <SingleNote key={i} note={note} />
